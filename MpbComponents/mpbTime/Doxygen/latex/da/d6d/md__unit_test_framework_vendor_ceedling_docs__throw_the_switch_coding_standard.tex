...
;)\hypertarget{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md54}{}\doxysection{Why Have A Coding Standard?}\label{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md54}
Being consistent makes code easier to understand. We\textquotesingle{}ve tried to keep our standard simple because we also believe that we can only expect someone to follow something that is understandable. Please do your best.\hypertarget{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md55}{}\doxysection{Our Philosophy}\label{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md55}
...
\item Consistent
\item Memorable
\end{DoxyEnumerate}\hypertarget{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md57}{}\doxysubsubsection{Readable}\label{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md57}
We want to read our code. This means we like names and flow that are more naturally read. We try to avoid double negatives. We try to avoid cryptic abbreviations (sticking to ones we feel are common).\hypertarget{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md58}{}\doxysubsubsection{Descriptive}\label{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md58}
...
We\textquotesingle{}ve decided on our own style preferences. If you\textquotesingle{}d like to contribute to these projects (and we hope that you do), then we ask if you do your best to follow the same. It will only hurt a little. We promise.\hypertarget{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md62}{}\doxysubsubsection{Whitespace}\label{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md62}
...
\end{DoxyCode}
\hypertarget{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md63}{}\doxysubsubsection{Case}\label{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md63}
...
\end{DoxyItemize}\hypertarget{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md64}{}\doxysubsubsection{Braces}\label{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md64}
...
\end{DoxyCode}
\hypertarget{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md65}{}\doxysubsubsection{Comments}\label{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md65}
Do you know what we hate? Old-\/school C block comments. BUT, we\textquotesingle{}re using them anyway. As we mentioned, our goal is to support every compiler we can, especially embedded compilers. There are STILL C compilers out there that only support old-\/school block comments. So that is what we\textquotesingle{}re using. We apologize. We think they are ugly too.\hypertarget{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md66}{}\doxysection{Ruby Details}\label{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md66}
Is there really such thing as a Ruby coding standard? Ruby is such a free form language, it seems almost sacrilegious to suggest that people should comply to one method! We\textquotesingle{}ll keep it really brief!\hypertarget{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md67}{}\doxysubsubsection{Whitespace}\label{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md67}
Our Ruby style is to use spaces and to use 2 of them per indent level. It\textquotesingle{}s a nice power-\/of-\/2 number that really grooves with Ruby\textquotesingle{}s compact style. We have no more reason than that. We break that rule when we have lines that wrap. When that happens, we like to indent further to line things up in nice tidy columns.\hypertarget{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md68}{}\doxysubsubsection{Case}\label{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md68}
...
\end{DoxyItemize}\hypertarget{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md69}{}\doxysection{Documentation}\label{md__unit_test_framework_vendor_ceedling_docs__throw_the_switch_coding_standard_autotoc_md69}
